...
> Chrome ウェブストアには公開していません。ローカル読み込みで使います。
...
2. ページ内に追加された **「スライドをPDFでダウンロードする」** ボタンをクリック
3. 新しいタブが開き、「スライドをPDFでダウンロードしますか？」という確認ダイアログが出るので **OK**
4. スライドの取り込みが終わると **Chrome の印刷ダイアログが自動で開く**
5. 「送信先」を **「PDFに保存」** にして **保存**

もし印刷ダイアログが自動で開かなかった場合は、ページ上部の緑色のバーにある **「印刷ダイアログを開く」** ボタンを押してください。
...
その後、`chrome://extensions/` の **INIAD++** カードで **再読み込み（⟳）** ボタンを押してください。
...
`chrome://extensions/` の **INIAD++** カードの **「削除」** ボタンを押すだけです。
...
2. **Console** タブで `[INIAD++ PDF]` で始まるログを確認
...
| 症状 | 確認すること |
| --- | --- |
| ボタンが出ない | INIAD++ が有効になっているか。講義ページを F5 でリロード |
| 印刷ダイアログが開かない | 置き換わったページ上部の緑バーの「印刷ダイアログを開く」ボタンを押す |
| 一部のスライドが空白になる | Console に `[INIAD++ PDF]` のログを出し、どの page で止まったかを Issue で報告 |
