...
# INIADPLUS (PDF / Image Download Fork)

**INIAD MOOCs の講義スライドを、講義単位でまとめて PDF / PNG 画像として保存できる Chrome 拡張**
...
**[akahoshi1421/INIAD-](https://github.com/akahoshi1421/INIAD-) のフォーク**で、**スライドのダウンロードを HTML 出力から PDF 出力に差し替え、さらに PNG 画像出力にも対応させた版**です。

- 拡張機能本体の全機能・背景・使い方は **[本家 README](https://github.com/akahoshi1421/INIAD-#readme)** を参照してください
- このリポジトリの README は **PDF / 画像 対応に関する差分と、それを使うためのインストール手順** だけを扱います
...
| | 本家 | 本フォーク |
| --- | --- | --- |
| スライドのダウンロード形式 | HTML | **PDF**（A4 横 / 1 ページ 1 スライド）または **PNG 画像**（1 スライド 1 ファイル） |
...
2. ページに追加されるボタンから保存形式を選びます
   - スライドが 1 つだけ: **「スライドをPDFでダウンロード」** / **「スライドを画像でダウンロード」**
   - 複数ある講義: **「この講義のスライドを一括PDFダウンロード」** / **「この講義のスライドを一括画像ダウンロード」**、または個別の **「資料1をPDFでダウンロード」** / **「資料1を画像でダウンロード」**
3. 拡張機能が講義内の Google Slides を順番に開き、PDF または PNG を自動でダウンロードします
   - PDF は **A4 横 / 1 ページ 1 スライド** で 1 ファイル
   - 画像は **1 スライド 1 ファイル**（`<タイトル>_p001.png`, `_p002.png` ... の連番）として保存
...
| 画像が自動保存されない | Chrome の「ダウンロード時に毎回確認」設定の有無、`debugger` 権限を許可しているかを確認。タブを閉じて再実行 |
